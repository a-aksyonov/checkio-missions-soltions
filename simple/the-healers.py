...
def fight_with_army(army_1: 'Army', army_2: 'Army'):
    class EmptyHealer(Healer):
        """Empty healer class to avoid many if-branches
        for the case when there is nobody behind unit
        """
        def heal(self, unit):
            pass

    healer_1 = EmptyHealer()
    healer_2 = EmptyHealer()
    unit_1 = army_1.get_fighter()
    if len(army_1.units_list) > 1:
        healer_1 = army_1.units_list[1]
    unit_2 = army_2.get_fighter()
    if len(army_2.units_list) > 1:
        healer_2 = army_2.units_list[1]
    while unit_1.is_alive and unit_2.is_alive:
